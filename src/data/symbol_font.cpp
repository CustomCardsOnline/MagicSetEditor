		...
		FOR_EACH_CONST(sym, symbols) {
			if (!sym->code.empty() && sym->enabled) {
				if (sym->regex) {
					...
					if (sym->code_regex.matches(results, text.c_str(), pos)
	...
	for (pos = start ; pos < text.size() ; ) {
		...
		FOR_EACH_CONST(sym, symbols) {
			if (!sym->code.empty() && sym->enabled) {
				if (sym->regex) {
					Regex::Results results;
					if (!sym->code_regex.empty() && sym->code_regex.matches(results, text.c_str(), pos)
							&& results.position() == 0 && results.length() > 0) { //Matches the regex
						pos += results.length();
						goto next_symbol;
					}
...
String InsertSymbolMenu::getCode(int id, const SymbolFont& font) const {
	if (type == ITEM_SUBMENU) {
		...
		return wxGetTextFromUser(message, caption).ToStdWstring();
	}
	return wxEmptyString;
}
